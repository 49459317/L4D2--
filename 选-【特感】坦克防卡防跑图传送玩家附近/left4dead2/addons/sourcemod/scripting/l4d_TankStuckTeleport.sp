...
public void OnPluginStart()
{
	...
	g_hCvarAllIntellect = CreateConVar("l4d_TankAntiStuck_all_intellect",				"1",	"防卡死应用的目标: 1=玩家和电脑人, 0=只有电脑人", CVAR_FLAGS);
	...
	g_hCvarRusherCheckInterv = CreateConVar("l4d_TankAntiStuck_rusher_check_interval",	"20",	"每次经过这么久(秒)检测一次玩家是否跑图", CVAR_FLAGS);
	g_hCvarRusherMinPlayers = CreateConVar("l4d_TankAntiStuck_rusher_minplayers",		"2",	"至少要这么多玩家才考虑检查是否有人跑图", CVAR_FLAGS);
	g_hCvarRusherEnable = CreateConVar("l4d_TankAntiStuck_rusher_Enable",				"1",	"救援关是否检测跑图[1=是, 0=否]", CVAR_FLAGS);
				...
				if (tank != 0) {
					...
					if (distance > g_hCvarRusherDist.FloatValue) {
						...
						if (g_iRushTimes[i] >= g_hCvarRusherCheckTimes.IntValue) {
							...
							pos[1] -= 20.0;
							pos[2] -= 20.0;
							PrintToChatAll("\x04[提示]\x05惩罚传送\x03坦克%s\x05瞬移到跑图玩家\x03%N\x05身边.", GetPlayerName(tank), i);
							g_iRushTimes[i] = 0;
							break;
						}
						else
...
void TeleportToObject(int client) {
	...
	if (target != 0 && IsPlayerAlive(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 8) {
		...
		pos[0] -= 50.0;
		pos[1] -= 50.0;
		PrintToChatAll("\x04[提示]\x05防卡启动\x03坦克%s\x05随机瞬移到玩家\x03%N\x05身边.", GetPlayerName(client), target);
	}
}
