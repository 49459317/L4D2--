...
public void OnPluginStart() {
	...
	g_cvMaxTankPlayer = CreateConVar("cz_max_tank_player", 							"2", 					"坦克玩家达到多少后插件将不再控制玩家接管(0=不接管坦克)", CVAR_FLAGS, true, 0.0);
	...
	g_cvSurvivorLimit = CreateConVar("cz_allow_survivor_limit", 					"0", 					"至少有多少名正常生还者(未被控,未倒地,未死亡)时,才允许玩家接管坦克", CVAR_FLAGS, true, 0.0);
	g_cvSurvivornumber = CreateConVar("cz_allow_survivor_number", 					"8", 					"生还者至少有多少人才能使用team3指令(包括BOT在内)", CVAR_FLAGS, true, 0.0);
	g_cvAutoaddteam3 = CreateConVar("cz_allow_whole_team", 							"0", 					"强迫所有玩家加入到感染者团队,全员特感模式(开启后sm_team2,sm_team3指令失效)0=关闭,1=开启", CVAR_FLAGS, true, 0.0);
	g_cvPZteamHealth = CreateConVar("cz_pz_team_health", 							"2.0", 					"特感玩家血量值设置(0.0=默认血量,数值=计算方式为当前血量乘以该数值)", CVAR_FLAGS, true, 0.0);
	...
	g_cvPZRespawnTime = CreateConVar("cz_pz_respawn_time", 							"3", 					"特感玩家自动复活时间(0=插件不会接管特感玩家的复活)", CVAR_FLAGS, true, 0.0);
	g_cvPZPunishTime = CreateConVar("cz_pz_punish_time", 							"0", 					"特感玩家在ghost状态下切换特感类型后下次复活延长的时间(0=插件不会延长复活时间)", CVAR_FLAGS, true, 0.0);
	g_cvPZPunishHealth = CreateConVar("cz_pz_punish_health", 						"0.0", 					"特感玩家在ghost状态下切换特感类型是否进行血量惩罚(0.0=不惩罚.计算方式为当前血量乘以该值)", CVAR_FLAGS, true, 0.0);
	g_cvAutoDisplayMenu = CreateConVar("cz_atuo_display_menu", 						"0", 					"在感染玩家进入灵魂状态后自动向其显示更改类型的菜单?(0=不显示,-1=每次都显示,大于0=每回合总计显示的最大次数)", CVAR_FLAGS, true, -1.0);
	g_cvPZTeamLimit = CreateConVar("cz_pz_team_limit", 								"-1", 					"感染玩家数量达到多少后将限制使用sm_team3命令(-1=感染玩家不能超过生还玩家,大于等于0=感染玩家不能超过该值)", CVAR_FLAGS, true, -1.0);
	g_cvCmdCooldownTime = CreateConVar("cz_cmd_cooldown_time", 						"0.0", 					"sm_team2,sm_team3命令的冷却时间(0.0-无冷却)", CVAR_FLAGS, true, 0.0);
	g_cvCmdEnterCooling = CreateConVar("cz_return_enter_cooling", 					"1", 					"什么情况下sm_team2,sm_team3命令会进入冷却(1=使用其中一个命令,2=坦克玩家掉控,4=坦克玩家死亡,8=坦克玩家未及时重生,16=特感玩家杀掉生还者玩家,31=所有)", CVAR_FLAGS);
	...
	g_cvUserFlagBits = CreateConVar("cz_user_flagbits", 							";z;z;z;z;;z", 			"哪些标志能绕过sm_team2,sm_team3,sm_pb,sm_tt,sm_pt,sm_class,鼠标中键重置冷却的使用限制(留空表示所有人都不会被限制)", CVAR_FLAGS);
	...
	g_cvSILimit = CreateConVar("cz_si_limit", 										"30", 					"同时存在的最大特感数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_SMOKER] = CreateConVar("cz_smoker_limit",					"2", 					"同时存在的最大smoker数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_BOOMER] = CreateConVar("cz_boomer_limit",					"2", 					"同时存在的最大boomer数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_HUNTER] = CreateConVar("cz_hunter_limit",					"6", 					"同时存在的最大hunter数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_SPITTER] = CreateConVar("cz_spitter_limit", 					"2", 					"同时存在的最大spitter数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_JOCKEY] = CreateConVar("cz_jockey_limit",					"6", 					"同时存在的最大jockey数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnLimits[SI_CHARGER] = CreateConVar("cz_charger_limit", 					"8", 					"同时存在的最大charger数量", CVAR_FLAGS, true, 0.0, true, 32.0);
	g_cvSpawnWeights[SI_SMOKER] = CreateConVar("cz_smoker_weight", 					"150", 					"smoker产生比重", CVAR_FLAGS, true, 0.0);
	g_cvSpawnWeights[SI_BOOMER] = CreateConVar("cz_boomer_weight", 					"150", 					"boomer产生比重", CVAR_FLAGS, true, 0.0);
	g_cvSpawnWeights[SI_HUNTER] = CreateConVar("cz_hunter_weight", 					"100", 					"hunter产生比重", CVAR_FLAGS, true, 0.0);
	g_cvSpawnWeights[SI_SPITTER] = CreateConVar("cz_spitter_weight", 				"150", 					"spitter产生比重", CVAR_FLAGS, true, 0.0);
	g_cvSpawnWeights[SI_JOCKEY] = CreateConVar("cz_jockey_weight", 					"100", 					"jockey产生比重", CVAR_FLAGS, true, 0.0);
	g_cvSpawnWeights[SI_CHARGER] = CreateConVar("cz_charger_weight", 				"200", 					"charger产生比重", CVAR_FLAGS, true, 0.0);
	...
	g_cvSurvivornumber.AddChangeHook(CvarChanged);
	g_cvAutoaddteam3.AddChangeHook(CvarChanged);
	g_cvPZteamHealth.AddChangeHook(CvarChanged);
...
void GetCvars_General() {
	...
	g_iSurvivornumber = g_cvSurvivornumber.IntValue;
	g_iSAutoaddteam3 = g_cvAutoaddteam3.IntValue;
	g_iSPZteamHealth = g_cvPZteamHealth.FloatValue;
...
Action cmdTeam2(int client, int args) {
	...
	if (g_iSAutoaddteam3 == 0) {
		ChangeTeamToSur(client);
	}
	else {
		PrintToChat(client, "\x04[提示]\x05全员特感模式时,使用该指令无效.");
	}
	return Plugin_Handled;
}
...
Action cmdTeam3(int client, int args) {
	...
	if (!CheckClientAccess(client, 1)) {
		...
		int team3 = _GetTeam3Count(3);
		int team2 = _GetTeam2Count(2);
		if ((g_iPZTeamLimit >= 0 && team3 >= g_iPZTeamLimit) || (g_iPZTeamLimit == -1 && team3 >= team2)) {
			PrintToChat(client, "\x04[提示]\x05已达到感染玩家数量限制.");
			return Plugin_Handled;
		}
		else if ((g_iPZTeamLimit >= 0 && g_iSurvivornumber > team2) || (g_iPZTeamLimit == -1 && g_iSurvivornumber > team2)) {
			PrintToChat(client, "\x04[提示]\x05幸存者数量至少有\x03%d\x05人才能用该指令加入感染者团队.", g_iSurvivornumber);
			return Plugin_Handled;
		}
	}
...
void Event_PlayerTeam(Event event, const char[] name, bool dontBroadcast) {
	...
	if (g_iSAutoaddteam3 != 0 && IsClientInGame(client) && !IsFakeClient(client) && team != 3) {
		CreateTimer(0.0, Autoaddteam3, client, TIMER_FLAG_NO_MAPCHANGE);
	}
}

Action Autoaddteam3(Handle timer, any client) {
	if (g_iSAutoaddteam3 != 0 && IsClientInGame(client) && !IsFakeClient(client)) {
		ChangeClientTeam(client, 3);
	}
	return Plugin_Handled;
}
...
void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast) {
	...
	if (g_iSPZteamHealth && IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") != 8) {
		SetEntityHealth(client, RoundToCeil(GetClientHealth(client) * g_iSPZteamHealth));
	}
}
...
void NextFrame_EnterGhostState(int client) {
	if (g_iControlled == 0 && (client = GetClientOfUserId(client)) && IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == 3 && IsPlayerAlive(client) && GetEntProp(client, Prop_Send, "m_zombieClass") != 8 && GetEntProp(client, Prop_Send, "m_isGhost")) {
		if (g_esPlayer[client].enteredGhost == 0) {
			if (CheckClientAccess(client, 0))
				CPrintToChat(client, "{olive}聊天栏输入 {red}!team2 {olive}可切换回 {red}生还者");
				
			if (CheckClientAccess(client, 5))
				CPrintToChat(client, "{olive}灵魂状态下按 {red}鼠标中键 {olive}换特感角色类型,按 {red}R {olive}可以灵魂飞行");
		}
